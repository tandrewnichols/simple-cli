...
exports.spawn = function(grunt, context, cmd, done) {
  ...
  builder.getDynamicValues(grunt, cmd, target, cmdOpts, function(err, cmdOpts) {
    ...
    if (options.stdio === false || options.stdout || options.stderr || options.stdin) {
      delete opts.stdio; 
    }
    ...
    ['stdout', 'stderr'].forEach(function(fd){
      if (child[fd]) {
        if (options[fd]) {
          if (typeof options[fd] === 'function') {
            child[fd].on('data', options[fd]);
          } else {
            child[fd].on(options[fd].event, options[fd].fn);
          }
        } else if (options.stdio !== false) {
