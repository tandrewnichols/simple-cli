...
exports.spawn = function(grunt, context, cmd, done) {
  ...
  builder.getDynamicValues(grunt, cmd, target, cmdOpts, function(err, cmdOpts) {
    ...
    var opts = { cwd: simple.cwd || process.cwd(), stdio: simple.stdio || 'inherit' };
    if (simple.stdio === false || simple.stdout || simple.stderr || simple.stdin) {
      delete opts.stdio; 
    }
    ...
    ['stdout', 'stderr'].forEach(function(fd){
      if (child[fd]) {
        if (simple[fd]) {
          if (typeof simple[fd] === 'function') {
            child[fd].on('data', simple[fd]);
          } else {
            child[fd].on(simple[fd].event, simple[fd].fn);
          }
        } else if (simple.stdio !== false) {
