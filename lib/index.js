...
exports.spawn = v('*s*s+osf', function(taskName, description, customOpts, cmd, cb) {
  return function(grunt) {
    grunt.registerMultiTask(cmd, description, function() {
      cmd = cmd || taskName;
      ...
      if (!_.isPlainObject(this.data)) {
        var parts = typeof this.data === 'string' ? this.data.split(' ') : this.data;
        ...
        async.each(_.keys(customOpts), function(opt, next) {
          ...
          if (debug) {
            ...
            if (_.keys(optObj.options).length) {
              child = spawn(optObj.cmd, [optObj.target].concat(optObj.args), optObj.options);
            } else {
              child = spawn(optObj.cmd, [optObj.target].concat(optObj.args));
            }
